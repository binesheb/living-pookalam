"""Runtime launcher: optional artwork, debug control, and warped artwork-aware effects."""
...
def interaction_with_effects(img,stop_event,debug_event):
 ...
 cap=cv2.VideoCapture(cfg['camera_index']); field=np.float32(cfg['projector_field_camera']); floor=np.float32(cfg['floor_boundary_camera']); Hfield=cv2.getPerspectiveTransform(field,np.float32([[0,0],[pw,0],[pw,ph],[0,ph]])); Hfloor=cv2.getPerspectiveTransform(floor,np.float32([[0,0],[640,0],[640,640],[0,640]])); Hcamproj=Hfield; Hfloorproj=Hfield@np.linalg.inv(Hfloor)
 ...
 artwork=None if img is None else cv2.resize(img,(640,640)); fx=EffectsEngine(pw,ph,artwork,Hfloorproj); kernel=np.ones((5,5),np.uint8); frame_count=0; tick=time.time(); fps=0
 while not stop_event.is_set():
  ...
  # Effects are generated on the square normalized floor, then warped through the exact calibration into projector pixels.
  base_floor=np.zeros((640,640,3),np.uint8) if artwork is None else artwork.copy(); fx_floor=fx.render_floor(base_floor); warped=cv2.warpPerspective(fx_floor,Hfloorproj,(pw,ph)); mask=cv2.warpPerspective(np.full((640,640),255,np.uint8),Hfloorproj,(pw,ph)); effect=stable.copy(); effect[mask>0]=warped[mask>0]; cv2.imshow(pname,effect)
  if debug:
   view=frame.copy(); cv2.polylines(view,[field.astype(np.int32)],True,(0,255,255),2); cv2.polylines(view,[floor.astype(np.int32)],True,(0,255,0),2); cv2.putText(view,f'DEBUG | FPS {fps:.1f} | FLOOR {len(interactions)}',(15,30),0,.65,(0,0,255),2); cv2.imshow('Debug - Camera',view); cv2.imshow('Debug - Scene Baseline',scene_base); cv2.imshow('Debug - Field Difference',fdiff); cv2.imshow('Debug - Field Mask',motion); cv2.imshow('Debug - Floor Live',floor_debug); cv2.imshow('Debug - Floor Baseline',floor_base); cv2.imshow('Debug - Floor Difference',rdiff); cv2.imshow('Debug - Floor Mask',rmask)
...
def project_optional(self):
 ...
 self.stop.clear(); img=None if self.image is None else cv2.cvtColor(np.array(self.image),cv2.COLOR_RGB2BGR); self.set_state('Starting warped artwork-aware effects — learning stable scene for 10 seconds',True)
 def work():

"""Runtime launcher: uploaded artwork or physical Pookalam detection."""
...
def interaction_with_effects(img,stop_event,debug_event):
 ...
 if img is not None:
  artwork=cv2.resize(img,(640,640)); vision={'mask':np.full((640,640),255,np.uint8),'center':(320,320),'radius':315,'confidence':1.0}; source='UPLOADED DESIGN'
 else:
  vision=detect_pookalam(floor_base); artwork=vision['artwork']; source=f"PHYSICAL POOKALAM {vision['confidence']:.0%}"
 fx=EffectsEngine(pw,ph,artwork,Hfloorproj)
 # Keep physical detection geometry authoritative for effects.
 fx.design_mask=vision['mask']; fx.cx,fx.cy=vision['center']; fx.radius=vision['radius']
 kernel=np.ones((5,5),np.uint8); tick=time.perf_counter(); frames=0; fps=0
 while not stop_event.is_set():
  ok,frame=cap.read()
  if not ok: continue
  debug=debug_event.is_set(); frames+=1
  now=time.perf_counter()
  if now-tick>=1: fps=frames/(now-tick);frames=0;tick=now
  fdiff,motion=app.difference(frame,scene_base); fieldmask=np.zeros(motion.shape,np.uint8);cv2.fillPoly(fieldmask,[field.astype(np.int32)],255);motion=cv2.morphologyEx(cv2.bitwise_and(motion,fieldmask),cv2.MORPH_OPEN,kernel)
  rectified=cv2.warpPerspective(frame,Hfloor,(640,640)); rdiff,rmask=app.difference(rectified,floor_base);rmask=cv2.morphologyEx(rmask,cv2.MORPH_OPEN,kernel);rmask=cv2.dilate(rmask,None,iterations=1);contours,_=cv2.findContours(rmask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE);floor_debug=rectified.copy();hits=0
  for q in contours:
...
def project_optional(self):
 ...
 self.stop.clear();img=None if self.image is None else cv2.cvtColor(np.array(self.image),cv2.COLOR_RGB2BGR);self.set_state('Learning physical scene for 10 seconds',True)
 def work():

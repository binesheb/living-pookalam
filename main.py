...
VERSION='2.1.0'; ROOT=Path(__file__).resolve().parent; CONFIG=ROOT/'calibration.json'
...
def calibration(idx):
    ...
    while True:
        ...
        label='PROJECTOR FIELD' if stage==0 else 'FLOOR AREA'; cv2.putText(frame,label+' | 4 corners | S next/save | U undo | R reset | ESC cancel',(15,30),0,.55,(255,255,255),2); cv2.imshow(win,frame); k=cv2.waitKey(1)&255
        if k==27: break
...
def build_projection(img,cfg):
    ...
    H=floor_to_proj@cv2.getPerspectiveTransform(src,unit); return cv2.warpPerspective(img,H,(pw,ph),borderValue=(0,0,0))

def projection_with_debug(img,stop_event):
    if not CONFIG.exists(): raise RuntimeError('Calibrate first')
    cfg=json.loads(CONFIG.read_text()); projected=build_projection(img,cfg); pname=projector_window('Living Pookalam - Projection',projected)
    cap=cv2.VideoCapture(cfg['camera_index']); field=np.float32(cfg['projector_field_camera']); floor=np.float32(cfg['floor_boundary_camera']); unit=np.float32([[0,0],[640,0],[640,640],[0,640]]); H=cv2.getPerspectiveTransform(floor,unit); prev=None
    while not stop_event.is_set():
        ...
        mask=np.zeros_like(gray); cv2.fillPoly(mask,[field.astype(np.int32)],255); motion=cv2.dilate(cv2.bitwise_and(motion,mask),None,iterations=2)
        rectified=cv2.warpPerspective(frame,H,(640,640)); rgray=cv2.cvtColor(rectified,cv2.COLOR_BGR2GRAY); rmask=cv2.threshold(rgray,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)[1]; contours,_=cv2.findContours(rmask,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE); n=0
        for q in contours:
            a=cv2.contourArea(q)
            if a<500: continue
            x,y,w,h=cv2.boundingRect(q); n+=1; cv2.rectangle(rectified,(x,y),(x+w,y+h),(0,255,0),2); cv2.putText(rectified,f'{(x+w/2)/640:.2f}, {(y+h/2)/640:.2f}',(x,y-5),0,.45,(0,255,0),1)
        cv2.polylines(frame,[field.astype(np.int32)],True,(0,255,255),2); cv2.polylines(frame,[floor.astype(np.int32)],True,(0,255,0),2); cv2.putText(frame,f'MOTION {np.count_nonzero(motion)} | OBJECTS {n}',(15,30),0,.65,(0,0,255),2)
        cv2.imshow('Camera - Projected Area',frame); cv2.imshow('Camera - Rectified Floor',rectified); cv2.waitKey(1)
    cap.release(); cv2.destroyAllWindows()
...
class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.title('Living Pookalam'); self.geometry('1120x780'); self.image=None; self.preview=None; self.stop=threading.Event(); self.worker=None
        top=tk.Frame(self,padx=24,pady=20); top.pack(fill='x'); tk.Label(top,text='LIVING POOKALAM',font=('Segoe UI',26,'bold')).pack(anchor='w'); tk.Label(top,text='Reference image → floor calibration → live camera understanding → projection effects',font=('Segoe UI',11)).pack(anchor='w')
        bar=tk.Frame(top); bar.pack(anchor='w',pady=16)
        for text,cmd in [('Select & Crop',self.select),('Calibrate',self.cal),('Start Projection',self.project),('Stop Projection',self.stop_projection),('Close App',self.close_app),('Update from GitHub',self.update)]: tk.Button(bar,text=text,command=cmd,width=17,height=2).pack(side='left',padx=3)
        self.status=tk.Label(top,text='Ready',anchor='w'); self.status.pack(fill='x'); self.view=tk.Label(self,text='REFERENCE IMAGE PREVIEW',font=('Segoe UI',16)); self.view.pack(expand=True,fill='both',padx=20,pady=20); self.protocol('WM_DELETE_WINDOW',self.close_app)
    def select(self):
        p=filedialog.askopenfilename(filetypes=[('Images','*.png *.jpg *.jpeg *.bmp *.webp')]);
        if p: Crop(self,p,self.setimg)
    def setimg(self,img): self.image=img; p=img.copy(); p.thumbnail((1000,560)); self.preview=ImageTk.PhotoImage(p); self.view.configure(image=self.preview,text=''); self.status.configure(text=f'Reference image ready: {img.width} x {img.height}')
    def cal(self):
        cs=cameras();
        ...
        if idx in cs:
            try: self.withdraw(); calibration(idx); self.status.configure(text='Calibration saved')
    ...
    def project(self):
        ...
        self.stop.clear(); img=cv2.cvtColor(np.array(self.image),cv2.COLOR_RGB2BGR); self.status.configure(text='Projection and live camera debug running')
        def work():
            try: projection_with_debug(img,self.stop)
            except Exception as e:self.after(0,lambda:messagebox.showerror('Projection',str(e)))
            finally:self.after(0,lambda:self.status.configure(text='Projection stopped'))
        self.worker=threading.Thread(target=work,daemon=True); self.worker.start()
    def stop_projection(self): self.stop.set(); self.status.configure(text='Stopping projection and camera debug...')
    def close_app(self):
        self.stop.set(); cv2.destroyAllWindows(); self.destroy()
    def update(self):

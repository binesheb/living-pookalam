...
class PatternProjectionWindow(ProjectionWindow):
    def render(self, interaction=None, contour=None, pattern=None, image=None):
        ...
        self.app.engine.render(DrawAdapter(self.canvas), self.w, self.h,
                               interaction=interaction, pattern=pattern)
...
class FieldExperienceConsole(FieldConsole):
    ...
    def open_projector(self):
        if self.proj is None or not self.proj.win.winfo_exists():
            self.proj=PatternProjectionWindow(self)
        self.proj.place()

    def _source_frame(self):
        # Uploaded image is analyzed when selected; physical/hybrid use the live camera.
    ...
    def analyse_pattern(self, force=False):
        ...
        try:
            ...
            self.contour=self.pattern.contour
            self.confidence=self.pattern.confidence
        except Exception:
            self.pattern=None
    ...
    def tick(self):
        ...
        if self.page=="analyze" and hasattr(self,"analysis_preview"):
            self._update_analysis_preview()

    def page_analyze(self):
        self.page_title("Analyse", "Understand the real Pookalam before assigning visual effects.")
        self.analyse_pattern(force=True)
        p=self.pattern
        ...
        for title,value,color in cards:
            ...
            tk.Label(f,text=title,bg=PANEL2,fg=MUTED,font=("Segoe UI",8,"bold")).pack(anchor="w",padx=12,pady=(10,2)); tk.Label(f,text=value,bg=PANEL2,fg=color,font=("Segoe UI",15,"bold")).pack(anchor="w",padx=12,pady=(0,10))
        bar=tk.Frame(self.main,bg=BG); bar.pack(fill="x",pady=8)
        self.button(bar,"ANALYSE NOW",lambda:self.analyse_pattern(True),True).pack(side="left",padx=4)
        self.button(bar,"SHOW REAL EDGE",self.show_edge).pack(side="left",padx=4)
        body=tk.Frame(self.main,bg=BG); body.pack(fill="both",expand=True)
        panel=tk.Frame(body,bg=PANEL2,highlightthickness=1,highlightbackground=BORDER); panel.pack(fill="both",expand=True)
        self.analysis_preview=tk.Label(panel,bg="#020204"); self.analysis_preview.pack(fill="both",expand=True,padx=8,pady=8)
        self._update_analysis_preview()
    ...
    def _update_analysis_preview(self):
        ...
        if p is not None and p.contour is not None:
            cv2.drawContours(frame,[p.contour],-1,(210,110,255),4)
            cx,cy=map(int,p.centre); cv2.circle(frame,(cx,cy),8,(0,255,255),-1)
            cv2.putText(frame,f"EDGE  CONF {p.confidence:.0%}  {p.symmetry_order}-FOLD",(20,35),cv2.FONT_HERSHEY_SIMPLEX,.75,(210,110,255),2)
    ...
    def page_experience(self):
        ...
        body=tk.Frame(self.main,bg=BG); body.pack(fill="both",expand=True)
        left=tk.Frame(body,bg=PANEL2,width=320,highlightthickness=1,highlightbackground=BORDER); left.pack(side="left",fill="y",padx=(0,6)); left.pack_propagate(False)
        ...
        for name in PRESETS:
            tk.Button(preset,text=name.replace("_"," "),command=lambda n=name:self.apply_preset(n),bg=PANEL3,fg=GOLD,relief="flat",font=("Segoe UI",8,"bold")).pack(fill="x",pady=2)
        scroll=tk.Canvas(left,bg=PANEL2,highlightthickness=0); scroll.pack(fill="both",expand=True,padx=8)
        inner=tk.Frame(scroll,bg=PANEL2); scroll.create_window((0,0),window=inner,anchor="nw")
        self.effect_vars={}
        for category in CATEGORIES:
            ...
            for spec in effects_by_category()[category]:
                var=tk.BooleanVar(value=bool(self.engine.effects.get(spec.id,False))); self.effect_vars[spec.id]=var
        ...
        self.intensity=tk.DoubleVar(value=self.engine.intensity*100); self.speed=tk.DoubleVar(value=self.engine.speed*100)

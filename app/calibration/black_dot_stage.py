...
def _white_field(self):
    ...
    self.canvas.create_text(self.w / 2, self.h * 0.48,
                            text="CALIBRATING PROJECTOR SPACE",
                            fill="#111111", font=("Segoe UI", 26, "bold"))
    self.canvas.create_text(self.w / 2, self.h * 0.54,
...
def _black_dot(self, index):
    ...
    self.canvas.create_oval(x-r * 0.16, y-r * 0.16, x+r * 0.16, y+r * 0.16,
...
def _detect_black_dot(frame: np.ndarray, expected_xy, radius: int):
    """Detect a dark target in a predicted camera-space ROI."""
...
def _page_calibrate(self):
    ...
    top = tk.Frame(self.main, bg="#090711")
    top.pack(fill="x")
    ...
    tk.Label(top, textvariable=self.calib_status, bg="#090711", fg="#ffd24a",
             font=("Segoe UI", 14, "bold")).pack(anchor="w")
    tk.Label(top, textvariable=self.calib_detail, bg="#090711", fg="#9c95aa",
             font=("Segoe UI", 9)).pack(anchor="w", pady=(2, 8))
    actions = tk.Frame(self.main, bg="#090711")
    ...
    body = tk.Frame(self.main, bg="#090711")
    body.pack(fill="both", expand=True)
    left = tk.Frame(body, bg="#171526", highlightthickness=1, highlightbackground="#2b2540")
    left.pack(side="left", fill="both", expand=True, padx=(0, 5))
    self.calib_preview = tk.Label(left, bg="#020204")
    self.calib_preview.pack(fill="both", expand=True, padx=8, pady=8)
    right = tk.Frame(body, bg="#171526", width=280, highlightthickness=1, highlightbackground="#2b2540")
    right.pack(side="right", fill="y", padx=(5, 0))
    right.pack_propagate(False)
    tk.Label(right, text="AUTOMATED SEQUENCE", bg="#171526", fg="#f4f1fa",
             font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=16, pady=14)
    self.calib_labels = []
    for i, name in enumerate(GEOMETRY_NAMES):
        var = tk.StringVar(value=f"{i + 1}   {name}   WAITING")
        lab = tk.Label(right, textvariable=var, bg="#171526", fg="#9c95aa",
                       font=("Consolas", 9, "bold"))
        lab.pack(anchor="w", padx=16, pady=8)
        self.calib_labels.append((var, lab))
    self.calib_progress = tk.StringVar(value="PROJECTOR SPACE")
    tk.Label(right, textvariable=self.calib_progress, bg="#171526", fg="#8b35ff",
             font=("Segoe UI", 21, "bold")).pack(anchor="w", padx=16, pady=8)
    self.calib_error = tk.StringVar(value="Reprojection error: PENDING")
    tk.Label(right, textvariable=self.calib_error, bg="#171526", fg="#9c95aa",
...
def _calibration_tick(self):
    ...
    if self.calib_index == -2:
        ...
        self.state["surface_rectangle_camera"] = self.projection_quad.tolist()
    ...
    if p is None:
        self.calib_history = []
        self.calib_detail.set(f"Geometry target {self.calib_index + 1}: acquiring black dot inside white field…")
        return
    self.calib_history.append(p)
    if len(self.calib_history) > 8:
        self.calib_history.pop(0)
    if len(self.calib_history) < 8:
        self.calib_detail.set(f"Geometry target {self.calib_index + 1}: pinpointing centre {len(self.calib_history)}/8…")
        return
    ...
    if refined is None:
        ...
        self.calib_error.set("Reprojection error: UNAVAILABLE • OLD MAP RETAINED")
        self.proj.black()
        return
    ...
    self.calib_error.set("Reprojection error: PENDING • GEOMETRY LOCKED")

...
def cameras(max_index: int = 10) -> dict:
    ...
    return {"selected_index":load_runtime_config()["camera_index"],"cameras":discover_cameras(max_index)}
...
def select_camera(selection: CameraSelection) -> dict:
    ...
    config = load_runtime_config(); config["camera_index"] = selection.index; save_runtime_config(config)
...
def get_manual_calibration() -> dict:
    return {"calibration":manual_calibration.as_dict() if manual_calibration else None,"corner_order":["top_left","top_right","bottom_right","bottom_left"]}

@app.post("/api/v1/calibration/manual")
def save_manual_calibration(request: ManualCornerRequest) -> dict:
    global manual_calibration
    try:
        manual_calibration = calibrate_manual_corners([[p.x,p.y] for p in request.points],request.physical_width_mm,request.physical_height_mm)

...
class ManualCornerRequest(BaseModel): points:list[Point]=Field(min_length=4,max_length=4); physical_width_mm:float=Field(gt=0); physical_height_mm:float=Field(gt=0)
class ProjectorCalibrationRequest(BaseModel): camera_floor_points:list[Point]=Field(min_length=4,max_length=4); projector_reference_camera_points:list[Point]=Field(min_length=4,max_length=4); physical_width_mm:float=Field(gt=0); physical_height_mm:float=Field(gt=0)
@app.on_event("startup")
def startup(): state.start()
@app.get("/calibrate",include_in_schema=False)
def calibrate_ui(): return FileResponse(Path(__file__).parent/'ui'/'calibration.html')
@app.get("/health")
def health(): p=load_profile(); return {"status":"ok","running":state.running,"version":__version__,"profile":p.get("id","unknown")}
...
def save_projector_calibration(request:ProjectorCalibrationRequest):
 ...
 try: floor_calibration=build_floor_calibration([[p.x,p.y] for p in request.camera_floor_points],[[p.x,p.y] for p in request.projector_reference_camera_points],request.physical_width_mm,request.physical_height_mm)
 except ValueError as e: raise HTTPException(400,str(e)) from e
